...
/**
 * m15 · 作用域注册 — 作用域原语（从 m13 复用）
 *
 * 一个 Scope 是父 context 下的一棵子树，带一个唯一的 scopeKey 标签。
 * Skill 系统就跑在各自的 agent scope 里：每个 agent 的已加载 skill
 * 缓存是作用域局部的，dispose 作用域时整体回收。
 *
 * 这里与 m13 完全一致：用 cordis 的 context 子树表达作用域。
 */
 ...
 * 它负责登记所有活跃作用域、按 key 取回、以及 dispose 某个作用域。
 * 自身挂在全局 ctx 上，所有作用域共享它。
 */
